  ...
  const nav = (
    <nav className="flex flex-1 flex-col gap-6 overflow-y-auto px-3 py-4" aria-label="Main">
      ...
      <div className="mt-auto px-1 pt-2">
        ...
        >
          <LogOut className="h-4 w-4" aria-hidden /> {!collapsed && "Logout"}
        </Button>
      </div>
    </nav>
  );
    ...
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 border-t border-sidebar-border py-3 text-xs font-medium text-muted-foreground hover:bg-muted"
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-card/90 px-4 backdrop-blur sm:px-6">
          ...
          <div className="relative hidden flex-1 md:block md:max-w-sm">
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                toast.info(query ? `Showing results for “${query}”` : "Type something to search");
              }}
            >
              <Input
                ...
                placeholder="Search students, courses, drives…"
                className="pl-9"
              />
            </form>
          </div>
