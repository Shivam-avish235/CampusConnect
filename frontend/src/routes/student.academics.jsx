...
function Academics(){
 const [results,setResults]=useState([]),[marks,setMarks]=useState([]);const id=Number(getCurrentSession()?.id);
 useEffect(()=>{Promise.all([resultsApi.list(),marksApi.list()]).then(([r,m])=>{setResults((r||[]).filter(x=>x.studentId===id));setMarks((m||[]).filter(x=>x.studentId===id))}).catch(e=>toast.error(e.response?.data?.message||"Unable to load academics"))},[]);
